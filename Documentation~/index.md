...
| `InputBlocker` | Disables the active `EventSystem` while the overlay is open, so uGUI stops raycasting. The type is resolved with `Type.GetType` rather than referenced, keeping ugui an optional dependency; the disabled `Behaviour` is remembered so it can be switched back on, including when the overlay is destroyed mid-session |
